...
class Graph<E> {
    ...
    *//************************ PRIM **************************************************************************//*
    ...
    List<Edge> prim(int start_index, int budget) {
        // lista koja kje gi sodrzi MST rebra
        List<Edge> mstEdges = new ArrayList<Edge>();
...
public class Railroads {
    public static void main(String[] args) throws IOException {
        ...
        String [] cityAB_price;
        String cityA,cityB,cityFinal;
        float price;

        for (int i = 0; i < m; i++){
            cityAB_price = br.readLine().split(" ");
            cityA = cityAB_price[0];
            cityB = cityAB_price[1];
            price = Float.parseFloat(cityAB_price[2]);

            graph.addEdge(table.get(cityA),table.get(cityB),price);
        }

        String [] city_budget = br.readLine().split(" ");
        cityFinal = city_budget[0];
        int budget = Integer.parseInt(city_budget[1]);

        List<Edge> list = graph.prim(table.get(cityFinal),budget);
        System.out.println(graph.prim(table.get(cityFinal),budget));
        int sumConnection = 0;
        int listSize = list.size();
        for (int i = 0; i < listSize; i++){
            float cityWeight = list.get(i).getWeight();
            sumConnection += cityWeight;
        }
        System.out.println(String.format("%d\n%d",sumConnection,listSize+1));
    }
